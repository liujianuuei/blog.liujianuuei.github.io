...
Multi-agent 是一种工程优化手段。对于复杂任务，最佳实践是建立一个**工作空间**（别忘了，记忆可以在工作空间共享），在工作空间里发起多个 subagent（即所谓AI员工） 进行工作，每个 subagent 负责处理任务的一小点。这样避免单一会话上下文爆炸。
...
*⒋ 比如专家、专家团、技能包、技能套件等；有的产品也称其为AI员工或 Agent（其实是 subagent），注意和运行时 [subagent](#-multi-agent) 概念的区别。*
