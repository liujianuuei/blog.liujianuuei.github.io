...
| Claude📍 | Anthropic | 美国 | Claude(网页/App)~~、Claude Cowork~~、Claude Code|
...
| 行业垂直应用 | 基于 Agent 技术、行业知识二次开发 | NA |
...
#### 🛠 Skills (and tools, and registry, and even plugins)
...
> *With Skills, you can teach Codex your team’s standards, ~~workflows~~processes, and ways of working. Codex applies them consistently across tasks, so it can contribute more effectively with less supervision. —— https://openai.com/codex/*
...
~~Multi-agent~~ Subagents 本质上就是**一套提示词和工具集**，解决的问题就是拆解任务，简化处理，缩小会话上下文，比如`~/.codex/agents/{name}.toml`、`{PROJECT_HOME}/.codex/agents/{name}.toml`。
...
| Claude Code ⛔<br>*CLI-Native Coding Agents* | 🇺🇸 | CLI / Desktop / Ext<br>*区域限制，无法测评* | ~~☆☆☆☆☆~~ |
...
| [Devin](https://devin.ai/) | 🇺🇸 | Desktop / CLI / Ext<br>*可使用组合模型模式* | ★★☆☆☆ |
...
找到（免费）模型提供商（[`阿里云百炼`](https://bailian.console.aliyun.com/)?） + Agent（`Claude Code?`）的最佳搭配。⏳
