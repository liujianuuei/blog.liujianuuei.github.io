...
[OpenClaw](https://openclaw.ai/) 是一个开源的大语言模型**执行框架**（智能体框架，Runtime Framework）。可以基于 OpenClaw 搭建甚至开发定制化的 Agent。不详述。

[Hermes Agent](https://hermes-agent.nousresearch.com/) 也是一个开源的大语言模型执行框架（智能体框架，Runtime Framework）。可以基于 Hermes Agent 搭建甚至开发定制化的 Agent。不详述。
