...
市面上有商业化 Agent 产品（办公的比如 WorkBuddy、编程的比如 Codex），我们可以使用，同时我们也可以基于开源项目自己搭建 Agent（自己搭建 Agent，只需要付模型的费用）。

OpenClaw 是一个开源的大语言模型**执行框架**。可以基于 [OpenClaw](https://openclaw.ai/) 搭建甚至开发定制化的 Agent。不详述。

Hermes Agent 也是一个开源的大语言模型**执行框架**。可以基于 [Hermes Agent](https://hermes-agent.nousresearch.com/) 搭建甚至开发定制化的 Agent。不详述。
