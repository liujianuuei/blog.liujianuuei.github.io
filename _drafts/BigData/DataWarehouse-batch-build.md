...
ODS+DW* 所有字段均为 `string` 类型（或者也可采用富类型：`string`、`bigint`、`int`、`decimal(38,6)`、`float`、`double`）。不允许有 `null`（应用 `coalesce()` 函数） 值，都转化成空串 `''` （空串一般不具有业务含义）。

ADS 字段类型按需，一般用 `string`，整数值用 `bigint` 或 `int`，涉及小数用 `decimal(38,6)` ~~或者 `double`~~。由于应用层涉及多种字段类型，无法统一把 `null` 转成空串 `''`，如果都转成相应类型默认值（比如 `0`），问题是这些默认值（比如 `0`）可能具有业务含义，故在应用层允许 `null` 存在。
