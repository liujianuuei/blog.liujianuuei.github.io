...
平台通过 Spark 集群分布式执行模型进行预测，并把预测结果写入Hive表。目前支持 Python Pickle（以.pkl结尾的文件）类型的模型文件。模型的加载执行是通过joblib完成的。
...
| 部署方式        | 说明      |
| :-----------   | :------- |
| 手动方式        |  模型文件、模型元数据和执行环境文件通过线下手动方式传递提供给大数据人员。     |
| 平台化方式      |   模型文件、模型元数据和执行环境文件通过线上平台化方式传递提供，模型人员通过程序（MLflow框架）在平台上（底层是阿里云OSS存储系统）生成这些文件，模型执行的时候自动读取生成的文件。     |
