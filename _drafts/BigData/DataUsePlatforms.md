...
上述所讲的都是指数据的应用。数据可以通过广义上的 API 的方式（比如：HDFS 文件地址、Kafka 所代表的的消息队列、SQL 和 NoSQL 数据库等）对外提供服务，即大数据服务平台。
...
变量服务是大数据提供服务的一种，变量可以是各种形式或者说存储于各种介质中，统一通过网关对外提供调用接口。

变量具体逻辑通过配置化的方式提供，比如在配置表里配置 MVEL 代码片段或其它实时变量（比如 StarRocks 变量等）代码片段，平台查时读取并执行，并返回结果。

对于 Hbase 等离线变量，提前在 Hive 里计算好变量值，然后加载到 Hbase 等离线 KV 介质中，平台查时直接读取返回。
...
模型由模型人员训练好，然后通过大数据 Spark 集群分布式执行模型进行预测，并把预测结果写入 Hive 表。

目前支持 Python Pickle（以`.pkl`结尾的文件）类型的模型文件。模型的加载执行是通过`joblib`完成的。
...
```bash

```
...
```sql

```
