...
①处理时间 - 处理程序执行时候的时间，处理时间又包括 Kafka 的到达时间、处理完成时间和 Flink 程序的处理时间等各种系统时间。

②事件时间（Event-time） - 事件发生时候的自然时间，非系统时间或人为设置时间。
...
准确**记录**（注意，对状态的更新是持续进行的）并**处理**状态对于计算结果的正确性和一致性至关重要。在故障或中断之后能够继续准确地更新状态是容错的关键。
...
在故障或中断之后能够继续准确地更新状态（状态一致性）是容错的关键。Flink 使用叫作**检查点**（checkpoint）的技术来做到状态的重置。
