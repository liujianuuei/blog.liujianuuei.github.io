...
Kafka 提供的高性能和**持久性**是 Flink 按预期工作的前提。尤其持久性可以让消息**重播**（replay），这个功能使得 Flink 可以对事件流中的某一部分进行**重新计算**（Time Travel - 时间旅行），而这是准确计算的必要前提。
...
②事件时间（Event-time），即事件发生时候的自然时间（编程中需要显式指定），非系统时间或人为设置时间。
...
Watermark 的生成不是基于单个事件的到达，而是基于**一个时间窗口内的事件到达情况**。在这个时间窗口内，只要有新的时间戳在 T 之前的事件到达，Flink 就会记录状态并继续等待更多的（可能乱序）事件。只有当时间**窗口结束**，且**没有更多的乱序事件**到达时（等待"最大允许乱序时间"后），才会生成 Watermark。

![](dw-flink-watermark.jpg)

#### 乱序

乱序的发生不可避免。可以通过 `forBoundedOutOfOrderness()` 方法设置"最大允许乱序时间"，以及 `withTimestampAssigner()` 设置事件的时间戳。

Watermark 生成的过程（或者先决条件）就已经考虑了乱序的处理——**只有等待超过"最大允许乱序时间"后，才会生成 Watermark**——也就是说，乱序事件会被包含进来。

所以，特别注意，最大允许乱序时间的设置会影响 Flink 处理的时效性，所以需要**在时效性和完整性上做平衡**。下面要讲的"最大允许迟到时间"则不会影响时效性。

#### 迟到

迟到的发生不可避免。迟到发生的时候，该事件所属窗口的 Watermark 已经生成。

对迟到事件的最简单直接的处理就是丢弃。或者，可以通过 `sideOutputLateData()` 方法对迟到的事件进行旁路输出。

再者，如果想要处理迟到事件（比如**更新**先前计算的结果），可以通过 `allowedLateness()` 方法设置"最大允许迟到时间"。如果一个事件在 `allowedLateness()` 指定的时间范围内到达，Flink 仍然会考虑并处理它；否则，该事件会被丢弃。
...
多流关联（特指 `connect()` 算子，`join()`、`between()` 算子语义复杂不详述）必然涉及一个流等待另一个流的到达的问题，关键在于先到的流数据如何保存——①共享实例变量②保存在状态里。——这是属于最佳实践的问题，可惜尚未深度使用。
