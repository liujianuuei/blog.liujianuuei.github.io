...
分布式环境下的锁机制。其实，只要在多实例环境下，不完全是分布式事务环境，就需要分布式锁。分布式锁的核心是多个实例的多个线程争夺唯一锁，这个锁只要是通过**单线程**（锁本身避免多线程控制）授予即可。基于 redis 的分布式锁只是其中一种实现。
...
```SQL
lock tables ... write; -- 这个肯定是表锁/排他(X)锁
select ... for share; -- 共享意向(IS)锁：其实详细过程是先获得表锁，然后查询出想要的记录，然后再加行锁，释放表锁
select ... for update; -- 排他意向(IX)锁：其实详细过程是先获得表锁，然后查询出想要的记录，然后再加行锁，释放表锁
insert into ... with AUTO_INCREMENT columns -- 如果 insert 语句包含自增 ID，则会加 AUTO-INC 锁，AUTO-INC 是表锁
```
...
行锁意味着在一个事务内，会发生**多次获得锁释放锁**的情况（当然，可能获得或释放的不是同一行）。

```SQL
select c1 from t where c1 = 10 for update; -- 加行锁
```

#### 间隙锁（gap lock）

间隙锁主要解决显式锁定情况下的幻读问题。锁定的是索引记录之间的空隙。

值得注意的是，共享间隙锁和排他间隙锁可以共存，相互并不冲突。排他间隙锁之间是不可共存的。

```SQL
select c1 from t where c1 between 10 and 20 for update; -- 加间隙锁
```

另外，执行插入语句的时候，会先获得一个所谓的*插入意向锁*，再获得插入记录的排他锁。

在 `READ COMMITTED` 事务级别，间隙锁关闭。

#### Next-Key 锁

Next-Key 锁是索引记录锁（即行锁）和索引记录之前的间隙锁的组合。Next-Key 锁的使用，要通过显式锁定来告诉存储引擎，比如下面的语句：
...
基于提升并发性能的考虑，大多数事务型存储引擎在实现锁的基础上，一般都同时实现了多版本并发控制。可以认为 MVCC 是锁的一个变种。

在一般情况下，比如非显式加锁的情况，存储引擎会选择使用 MVCC 来做并发控制。
...
死锁的发生是由于小于事务粒度即表粒度的锁机制造成的。由于**行锁锁定的永远是索引记录**，在多线程环境下，如果*没有为 DDL 语句的 where 字段建立联合索引*，则也有可能发生死锁，即便线程间根据 where 字段过滤出的最终记录并不冲突。
