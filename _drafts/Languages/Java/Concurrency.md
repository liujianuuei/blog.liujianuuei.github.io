...
除了获得锁的线程能进入该对象之外，其它请求同一个对象的线程都会被挂起，无论请求的是不是同一个方法或同一块代码，这种结构在并发理论中被称为*临界区（Critical Section）*。`synchronized` 可*重入*。
...
数据库也是程序的一个环节，任何适用于程序逻辑的锁，同样适用于数据库编程的并发控制，比如Java的互斥量式的块同步，以及分布式锁等。如果程序本身是线程安全的，那么数据库肯定也是线程安全的。常用。

#### 乐观锁

乐观锁是外部加于数据库的锁机制。通过一个预置的状态或版本校验，来判断当前行的数据是否是预期的，是否被改过。常用。
...
表锁是锁定整张表，一个线程/事务写入前需要获得该锁，会阻塞其它线程/事务对该表的所有读写操作。读之间是不互相阻塞的。

存储引擎会管理自己的锁，决定什么时候使用什么锁，MySQL 服务器会为诸如`ALTER TABLE`之类的语句使用表锁，而忽略存储引擎的锁机制。
