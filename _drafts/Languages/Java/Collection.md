...
## 所谓的 Fail-fast/Fail-safe

在使用迭代器遍历一个集合对象时，如果遍历过程中删除集合元素，则会抛出 `ConcurrentModificationException` 异常，这是因为迭代器在遍历的时候，会核查集合对象的元素个数，如果发生变化则抛出异常。所以，在遍历的时候应尽量避免对元素数目造成影响的改动，如必须要删除一个元素，则首先使用显式迭代器，然后用迭代器本身的方法进行操作，即 `Iterator.remove()`。`java.util` 包下的集合类都是快速失败机制的，不能再多线程环境下使用。

采用安全失败机制的集合类，在遍历时不是直接在集合上访问，而是先复制，再在复制的集合对象上进行遍历。所以不会造成不一致的冲突。J.U.C 包下的集合类都是安全失败机制的，可以在多线程下并发使用和修改。
